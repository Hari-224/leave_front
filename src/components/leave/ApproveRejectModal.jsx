...
import { X, CheckCircle, XCircle, MessageSquare, ThumbsUp, ThumbsDown, AlertTriangle, Eye, Calendar, Clock, User, FileText } from "lucide-react";
import "./ApproveRejectModal.css";

const ApproveRejectModal = ({ leave, onApprove, onReject, onClose, isOpen = true }) => {
  ...
  const resetState = () => {
    setReason(""); setAction(null); setErrors({}); setShowConfirmation(false); setIsSubmitting(false);
  };

  const validateReason = (text) => {
    const err = {};
    if (action === "reject") {
      if (!text.trim()) err.reason = "Please provide a reason";
      else if (text.trim().length < 10) err.reason = "Reason must be at least 10 chars";
      else if (text.trim().length > 500) err.reason = "Reason cannot exceed 500 chars";
    }
    setErrors(err);
    return Object.keys(err).length === 0;
  };

  const handleAction = (type) => { setAction(type); setShowConfirmation(true); validateReason(reason); };
  const handleConfirm = async () => {
    if (!validateReason(reason)) return;
    setIsSubmitting(true);
    try {
      if (action === "approve") await onApprove(leave.id);
      else await onReject(leave.id, reason.trim());
      onClose?.();
    } catch (e) {
      setErrors({ submit: e.message || "Failed to process request" });
    } finally { setIsSubmitting(false); setShowConfirmation(false); }
  };
  const handleCancel = () => { if (isSubmitting) return; setShowConfirmation(false); setAction(null); setErrors({}); };

  if (!isOpen || !leave) return null;

  const formatDate = (date) => new Date(date).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });
  const daysCount = () => Math.ceil(Math.abs(new Date(leave.endDate) - new Date(leave.startDate)) / (1000 * 60 * 60 * 24)) + 1;

  const getColor = (type, status) => {
    const leaveColors = { SICK: "red", VACATION: "blue", PERSONAL: "purple", EMERGENCY: "orange" };
    const statusColors = { PENDING: "yellow", APPROVED: "green", REJECTED: "red" };
    return type ? leaveColors[type] || "gray" : statusColors[status] || "gray";
  };
          ...
          {!showConfirmation ? (
            <div className="arm-actions">
              <textarea 
                ...
                disabled={isSubmitting} className={errors.reason ? "arm-input-error" : ""}
              />
            ...
            <div className={`arm-confirm ${action === "approve" ? "arm-confirm-approve" : "arm-confirm-reject"}`}>
              ...
              <div className="arm-buttons">
                <button onClick={handleConfirm} disabled={isSubmitting}>{isSubmitting ? "Processing..." : `Yes, ${action}`}</button>
