...
const LeaveTable = ({ onSelectLeave, onViewLeave, onEditLeave }) => {
  ...
  const getStatusIcon = (status) =>
    ...
    }[status?.toUpperCase()] || <AlertCircle className="icon-gray" />);

  const getStatusBadge = (status) =>
    `badge badge-${status?.toLowerCase() || "gray"}`;
  ...
  const toggleSelectAll = (checked) => {
    setSelectedLeaves(
      checked ? new Set(filteredAndSortedLeaves.map((l) => l.id)) : new Set()
    );
  };
  ...
  const handleBulkAction = async (action) => {
    ...
    try {
      await Promise.all(
        Array.from(selectedLeaves).map((id) => updateLeaveStatus(id, action, token))
      );
      fetchLeaves();
      setSelectedLeaves(new Set());
    } catch (err) {
      console.error(`Error performing ${action} action:`, err);
      alert(err.response?.data?.message || err.message || `Failed to ${action} leaves`);
    }
  };
  ...
  return (
    <div className="leave-table">
      <div className="header">
        ...
        <div>
          {filteredAndSortedLeaves.length} of {leaves.length} requests{" "}
              ...
              <tr>
                <th>
                  <input
                    type="checkbox"
                    checked={
                      selectedLeaves.size === filteredAndSortedLeaves.length &&
                  ...
                  <tr key={leave.id}>
                    <td>
                      <input
                        ...
                        onChange={(e) => toggleSelectLeave(leave.id, e.target.checked)}
                      />
                    </td>
                    <td>{leave.id}</td>
                    <td>{leave.employeeName}</td>
                    <td>{leave.leaveType}</td>
                    <td className={getStatusBadge(leave.status)}>
                    ...
                    <td className="actions">
                      <button onClick={() => onViewLeave(leave)} title="View">
                        <Eye />
                      </button>
                      <button onClick={() => onEditLeave(leave)} title="Edit">
                        <Edit />
                      </button>
                      <button onClick={() => onSelectLeave(leave)} title="More">
                        <MoreHorizontal />
                      </button>
                    </td>
                  </tr>
      ...
      {selectedLeaves.size > 0 && (
        <div className="bulk-actions">
          ...
          <button onClick={() => handleBulkAction("REJECTED")}>Reject Selected</button>
        </div>
      )}
    </div>
  );
