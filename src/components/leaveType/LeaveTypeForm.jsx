// src/components/leaves/LeaveTypeForm.jsx
...
import { createLeaveType, updateLeaveType } from "../../api/leaveTypeApi";
import "./LeaveTypeForm.css";

const LeaveTypeForm = ({ onCancel, initialData = {}, mode = "create" }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      let result;
      if (mode === "create") {
        result = await createLeaveType(payload, token);
      } else {
        result = await updateLeaveType(initialData.id, payload, token);
      }

      alert(`Leave type ${mode === "create" ? "created" : "updated"} successfully!`);
      if (onCancel) onCancel(result);

    } catch (error) {
      console.error(error);
      setApiError(error.response?.data?.message || error.message || "Failed to save leave type. Please try again.");
    } finally {
      ...
      <form className="form-body" onSubmit={handleSubmit}>
              ...
              <div className="color-options">
                {colorOptions.map(c => (
                  <button
                    ...
                    className={formData.color === c ? "selected" : ""}
        ...
        <div className="form-actions">
          ...
          <button type="submit" disabled={isSubmitting}>
            {isSubmitting ? (mode === "create" ? "Creating..." : "Updating...") : (mode === "create" ? "Create Leave Type" : "Update Leave Type")}
          </button>
        </div>
      </form>
    </div>
